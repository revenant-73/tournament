...
- `src/lib/`: Core logic and third-party integrations, including `db/` (Drizzle client) and `scoring.js` utilities.
...
- Production deployments are automatically triggered on push to the `main` branch.
- Environment variables (`VITE_TURSO_CONNECTION_URL`, `VITE_TURSO_AUTH_TOKEN`) must be configured in the Vercel dashboard.
